...
int memberderf(void **mem1, unsigned int len1, void *mem2, unsigned int len2);
...
int main(int ARGC, char **ARGV)
{
    ...
    unsigned long combination_count = 2598960;  // TODO: cheating


// uint_32_t[0b110111110111110111110111110111 + 1]    // actually need
// to malloc this but no more pointers to strngs and no more hash
// table, just set high bits throughought big array to mark empty
// vals, write vals to big array, compact, sort, and translate as
// printing out.
    
    char **cardlists;

    if (!(cardlists = (char**)(malloc(sizeof(char*) * combination_count))))
    {
	fprintf(stderr, "Not enough memory to allocate combinations pointer array on heap.\n");
	exit(1);
    }
			    ...
			    for (k2 = 'A'; k2 <= 'D'; k2++)
					...
					for (l2 = 'A'; l2 <= 'D'; l2++)
						...
						for (m2 = 'A'; m2 <= 'D'; m2++)
						{
							...
							&& (flag = 1) && !in_hash_table_p(cards)

							)
						    {
							if (c > combination_count)
							{
							    fprintf(stderr, "Too many combinations - off end of 'cardlists' array.\n");
							    fprintf(stderr, "(cards = %s)\n", cards);
							    exit(1);
							}
							    
							if (!(cardlists[c] = (char*)(malloc((sizeof(char) * CARD_WIDTH * NUM_CARDS + 1)))))
							{
							    fprintf(stderr, "Not enough memory to allocate string for combination %lu\n", c);
							    exit(1);
							}
							strcpy(cardlists[c], cards);
							add_to_hash_table(cardlists[c]);
							c++;
						    }
						}

    // Note just sorting the pointers!
    //qsort(cardlists, combination_count, sizeof(char*), handcompare);
        insertion_sort(cardlists, combination_count, sizeof(char*), handcompare);     

    for (c = 0; c < combination_count; c++)
    {
	printf("%s\n", cardlists[c]);
	free(cardlists[c]);
    }
...
/* Return 1 if mem2, which has length len2, is found anywhere in the
   "mem2" size blocks that mem1 is an array of len1 pointers to */
int memberderf(void **mem1, unsigned int len1, void *mem2, unsigned int len2)
{
    char **p;
    
    for (p = (char**)mem1; p < (char**)mem1 + len1; p ++)
	if (!memcmp(*p, (char*)mem2, len2))
	    return 1;
    return 0;
}
